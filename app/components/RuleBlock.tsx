...
type Rule = {
  id: number;
  indicator?: keyof typeof conditionOptionsMap;
  condition?: string;
  value?: string;
};

// Maps for dropdown options
const indicatorOptions = ['RSI', 'MACD', 'Moving Average', 'Candle'] as const;
...
const RuleBlock: React.FC<RuleBlockProps> = ({ rule, onDelete, onUpdate }) => {
  const handleIndicatorChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newIndicator = e.target.value as Rule['indicator'];
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="flex flex-col flex-1">
          ...
          <select
            ...
            className="bg-slate-800 p-2 rounded-md text-white outline-none"
            disabled={!rule.indicator}
          >
            <option value="" disabled>Select condition</option>
            {rule.indicator &&
              conditionOptionsMap[rule.indicator]?.map((opt) => (
                <option key={opt} value={opt}>
                  {opt}
                </option>
              ))}
        ...
        <div className="flex flex-col flex-1">
          ...
          >
            ...
            {rule.indicator &&
              valueOptionsMap[rule.indicator]?.map((opt) => (
                <option key={opt} value={opt}>
                  {opt}
                </option>
              ))}
