  ...
  return (
    <div className="bg-slate-700 rounded-xl p-4 flex flex-col gap-4 text-white shadow-md">
        <div className="grid grid-cols-[1fr_auto_auto] items-center gap-2">
            <select value={rule.indicator || ''} onChange={handleIndicatorChange} className="bg-slate-800 p-2 rounded-md font-semibold w-full">
                <option value="" disabled>Select Indicator</option>
                {Object.keys(INDICATOR_CONFIG).map(opt => <option key={opt} value={opt}>{opt}</option>)}
            </select>
            {config && config.params.length > 0 && (
                <button onClick={() => setShowSettings(!showSettings)} className={`p-2 rounded-md transition-colors ${showSettings ? 'bg-emerald-500' : 'bg-slate-600 hover:bg-slate-500'}`}>
                    <FiSettings />
                </button>
            )}
        ...
        {canHaveOffset && (
            <div className="mt-2 p-3 bg-slate-600/50 rounded-lg">
                <label className="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" className="h-4 w-4 rounded text-emerald-500 bg-slate-800 border-slate-600 focus:ring-emerald-500" checked={useOffset} onChange={() => setUseOffset(!useOffset)} />
