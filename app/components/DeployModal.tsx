...
export interface AllocationConfig {
  ...
  stocks: {
    symbol: string;
    amount?: number; // Only for static allocation
  }[];
}
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input 
                  ...
                  placeholder="e.g., INFY.BSE, press Enter"
              />
          ...
          <div>
            ...
            <div className="flex bg-slate-700 rounded-lg p-1">
              <button onClick={() => setAllocationType('dynamic')} className={`flex-1 flex items-center justify-center gap-2 text-sm p-2 rounded-md ${allocationType === 'dynamic' ? 'bg-emerald-500' : 'hover:bg-slate-600'}`}>
                <FiActivity /> Dynamic
              </button>
              <button onClick={() => setAllocationType('static')} className={`flex-1 flex items-center justify-center gap-2 text-sm p-2 rounded-md ${allocationType === 'static' ? 'bg-emerald-500' : 'hover:bg-slate-600'}`}>
                <FiDollarSign /> Static
              </button>
            </div>
          </div>
            ...
            <div className="space-y-2">
              ...
              {stocks.map(stock => (
                <div key={stock} className="flex items-center gap-2">
                  ...
                  <input 
                    type="number" 
                    value={staticAllocations[stock] || ''}
                    onChange={(e) => setStaticAllocations({...staticAllocations, [stock]: Number(e.target.value)})}
                    className="w-2/3 bg-slate-700 p-2 rounded-md" 
