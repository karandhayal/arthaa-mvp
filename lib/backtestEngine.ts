...
export function runBacktest(
  ...
  // ====================================================================
  // --- This is the new, more complete condition checker function ---
  // ====================================================================
  const checkCondition = (condition: any, candle: EnrichedCandle, prevCandle?: EnrichedCandle): boolean => {
    ...
    switch (indicator) {
      ...
      case 'Candle': {
        ...
        // Note: Engulfing patterns are more complex and omitted for this example.
        break;
      }
    }
  ...
  // --- Main Simulation Loop (No changes here) ---
  for (let i = 1; i < enrichedData.length; i++) {
    ...
    if (position) {
      ...
      const shouldExit =
        ...
        (exitConditions.length > 0 && exitConditions.every((cond: any) => checkCondition(cond, candle, prevCandle)));
    ...
    if (!position) {
      const shouldEnter = entryConditions.length > 0 && entryConditions.every((cond: any) => checkCondition(cond, candle, prevCandle));
      if (shouldEnter) {
  ...
  // --- Final Calculations (No changes here) ---
